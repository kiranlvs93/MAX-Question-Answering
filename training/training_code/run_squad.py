...
# Required parameters
flags.DEFINE_string("MODEL_DOWNLOAD_BASE", None, "The base url hosting the model.")
flags.DEFINE_string("MODEL_FILE", None, "The name of the model file.")
flags.DEFINE_string("MODEL_FOLDER", None, "The name of the model folder.")
...
flags.DEFINE_string("output_file_name", "predictions", "What file name predictions are written to.")
    ...
    for entry in input_data:
        for paragraph in entry["paragraphs"]:
            ...
            for qa in paragraph["qas"]:
                qas_id = qa["id"]
                print("..... Processing Qas " + str(qas_id))
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        print("..... Converting example " + str(example_index))
...
def main(_):
    ...
    archive_path = os.path.join("temp", model_file)
    folder_path = os.path.join("temp", model_folder)
    ...
    # load in the `bert_config.json` file
    bert_config = modeling.BertConfig.from_json_file(os.path.join(folder_path, 'bert_config.json'))
    validate_flags_or_throw(bert_config)

    # create output directory
    tf.gfile.MakeDirs(FLAGS.output_dir)

    # load in `vocab.txt` and initialize the tokenizer
    tokenizer = tokenization.FullTokenizer(
        vocab_file=os.path.join(folder_path, 'vocab.txt'), do_lower_case=FLAGS.do_lower_case)
